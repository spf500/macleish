...
PROOF THAT THE POINTS ARE IN ORDER ORIGINALLY:
...
```{r}
porcupine_trail_only <- Macleish_trails %>%
  filter(name == "Porcupine Trail") 
```

```{r}
porcupine_trail_cast <- Macleish_trails %>%
  filter(name == "Porcupine Trail") %>% 
  st_cast("MULTIPOINT")%>%
  st_cast("POINT")%>%
  mutate(id = row_number())
...
```{r, message = FALSE, warning = FALSE}
porcupine_intersections <- porcupine_trail_only %>%
  st_intersection(Contours) %>%
  st_cast("MULTIPOINT")%>%
  st_cast("POINT")
```
...
```{r}
porcupine_elev_ordered <-
  st_join(porcupine_trail_cast, porcupine_intersections)
```
...
my_points <- tribble(
  ~point, ~lon, ~lat,
  "A", -72.6836662428764, 42.453542675534,
  "B", -72.6833819549775, 42.4523518432371
